...
export const ConfigInventory = () => {
  ...
  if (inventario.length > 0) {
    inventarioSelect = inventario.find(async (item) => item._id === id);
    console.log("r", inventarioSelect);
    console.log("x", inventario);
  } else {
         ...
         mx-auto w-fit h-fit "
          >
            ...
            <p className="text-gray-500 mx-2">
              asigna bodegas tus usuarios facilitando la gestion de las bodegas
            </p>
            <div className="content grid grid-cols-2">
              {usersAdmin.length > 0 ? (
                ...
                <h1 className="text-center text-xl">No Tienes usuarios aun</h1>
              )}
            </div>
          </div>
        </div>
